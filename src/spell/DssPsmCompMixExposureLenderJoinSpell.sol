...
contract SpellAction {
    ...
    bytes32 constant ILK_PSM_COMP_LEND_USDC_A = "PSM-COMP-LEND-USDC-A";
...
// decimals & precision
    ...
    function execute() external limited {
        ...
        // Create Psm cme / lender join
        {
            ...
            JugAbstract(MCD_JUG).init(ILK_PSM_COMP_LEND_USDC_A);
        ...
        // create flip for Join lender USDC and DAI
        {
            ...
            address MCD_FLIP_PSM_COMP_LEND_USDC_A = address(new Flipper(MCD_VAT, MCD_CAT, ILK_PSM_COMP_LEND_USDC_A));
            address MCD_FLIP_PSM_COMP_LEND_DAI_A  = address(new Flipper(MCD_VAT, MCD_CAT, ILK_PSM_COMP_LEND_DAI_A));
            address PIP_DAI = address(new DSValue());


            require(FlipAbstract(MCD_FLIP_PSM_COMP_LEND_USDC_A).vat() == MCD_VAT, "flip-vat-not-match");
            require(FlipAbstract(MCD_FLIP_PSM_COMP_LEND_USDC_A).cat() == MCD_CAT, "flip-cat-not-match");
            require(FlipAbstract(MCD_FLIP_PSM_COMP_LEND_USDC_A).ilk() == ILK_PSM_COMP_LEND_USDC_A, "flip-ilk-not-match");
            require(FlipAbstract(MCD_FLIP_PSM_COMP_LEND_DAI_A).vat() == MCD_VAT, "flip-vat-not-match");
            require(FlipAbstract(MCD_FLIP_PSM_COMP_LEND_DAI_A).cat() == MCD_CAT, "flip-cat-not-match");
            require(FlipAbstract(MCD_FLIP_PSM_COMP_LEND_DAI_A).ilk() == ILK_PSM_COMP_LEND_DAI_A, "flip-ilk-not-match");


            // Set the USDC/DAI PIP in the Spotter
            SpotAbstract(MCD_SPOT).file(ILK_PSM_COMP_LEND_USDC_A, "pip", PIP_USDC);
            SpotAbstract(MCD_SPOT).file(ILK_PSM_COMP_LEND_DAI_A, "pip", PIP_DAI);

            // Set the PSM-USDC-A Flipper in the Cat
            CatAbstract(MCD_CAT).file(ILK_PSM_COMP_LEND_USDC_A, "flip", MCD_FLIP_PSM_COMP_LEND_USDC_A);
            CatAbstract(MCD_CAT).file(ILK_PSM_COMP_LEND_DAI_A, "flip", MCD_FLIP_PSM_COMP_LEND_DAI_A);


            // Update PSM-USDC-A spot value in Vat
            SpotAbstract(MCD_SPOT).poke(ILK_PSM_COMP_LEND_USDC_A);
            SpotAbstract(MCD_SPOT).poke(ILK_PSM_COMP_LEND_DAI_A);

            // Allow list PSM - USDC
            CatAbstract(MCD_CAT).rely(MCD_FLIP_PSM_COMP_LEND_USDC_A);
            FlipAbstract(MCD_FLIP_PSM_COMP_LEND_USDC_A).rely(MCD_CAT);
            FlipAbstract(MCD_FLIP_PSM_COMP_LEND_USDC_A).rely(MCD_END);
            FlipAbstract(MCD_FLIP_PSM_COMP_LEND_USDC_A).rely(FLIPPER_MOM);
            // Disallow Cat to kick auctions in PSM-USDC-A Flipper
            // !!!!!!!! Only for certain collaterals that do not trigger liquidations like USDC-A)
            FlipperMomAbstract(FLIPPER_MOM).deny(MCD_FLIP_PSM_COMP_LEND_USDC_A);

            // Allow list PSM - DAI
            CatAbstract(MCD_CAT).rely(MCD_FLIP_PSM_COMP_LEND_DAI_A);
            FlipAbstract(MCD_FLIP_PSM_COMP_LEND_DAI_A).rely(MCD_CAT);
            FlipAbstract(MCD_FLIP_PSM_COMP_LEND_DAI_A).rely(MCD_END);
            FlipAbstract(MCD_FLIP_PSM_COMP_LEND_DAI_A).rely(FLIPPER_MOM);
            // Disallow Cat to kick auctions in PSM-USDC-A Flipper
            FlipperMomAbstract(FLIPPER_MOM).deny(MCD_FLIP_PSM_COMP_LEND_DAI_A);

            IlkRegistryAbstract(ILK_REGISTRY).add(address(MCD_JOIN_COMP_LEND_USDC_A));
            IlkRegistryAbstract(ILK_REGISTRY).add(address(MCD_JOIN_COMP_LEND_DAI_A));

            CHANGELOG.setAddress("MCD_FLIP_PSM_COMP_LEND_DAI_A", MCD_FLIP_PSM_COMP_LEND_DAI_A);
            CHANGELOG.setAddress("MCD_FLIP_PSM_COMP_LEND_USDC_A", MCD_FLIP_PSM_COMP_LEND_USDC_A);
...

        {
            ...
            address MCD_CAT                   = CHANGELOG.getAddress("MCD_CAT");
            address MCD_JUG                   = CHANGELOG.getAddress("MCD_JUG");
            address MCD_FLIP_PSM_COMP_LEND_USDC_A  = CHANGELOG.getAddress("MCD_FLIP_PSM_COMP_LEND_USDC_A");
            address MCD_FLIP_PSM_COMP_LEND_DAI_A   = CHANGELOG.getAddress("MCD_FLIP_PSM_COMP_LEND_DAI_A");
            address MCD_SPOT                  = CHANGELOG.getAddress("MCD_SPOT");

            // Set PSM-COMP-USDC-A generic param
            VatAbstract(MCD_VAT).file(ILK_PSM_COMP_LEND_USDC_A, "line", 500 * THOUSAND * RAD);
            CatAbstract(MCD_CAT).file(ILK_PSM_COMP_LEND_USDC_A, "dunk", 50 * THOUSAND * RAD);
            CatAbstract(MCD_CAT).file(ILK_PSM_COMP_LEND_USDC_A, "chop", 113 * WAD / 100);
            JugAbstract(MCD_JUG).file(ILK_PSM_COMP_LEND_USDC_A, "duty", ZERO_PERCENT_RATE);
            FlipAbstract(MCD_FLIP_PSM_COMP_LEND_USDC_A).file("beg", 103 * WAD / 100);
            FlipAbstract(MCD_FLIP_PSM_COMP_LEND_USDC_A).file("ttl", 6 hours);
            FlipAbstract(MCD_FLIP_PSM_COMP_LEND_USDC_A).file("tau", 6 hours);
            SpotAbstract(MCD_SPOT).file(ILK_PSM_COMP_LEND_USDC_A, "mat", 100 * RAY / 100);

            // Set the PSM-LENDER-DAI-A generic param
            VatAbstract(MCD_VAT).file(ILK_PSM_COMP_LEND_DAI_A, "line", 500 * THOUSAND * RAD);
            CatAbstract(MCD_CAT).file(ILK_PSM_COMP_LEND_DAI_A, "dunk", 50 * THOUSAND * RAD);
            CatAbstract(MCD_CAT).file(ILK_PSM_COMP_LEND_DAI_A, "chop", 113 * WAD / 100);
            JugAbstract(MCD_JUG).file(ILK_PSM_COMP_LEND_DAI_A, "duty", ZERO_PERCENT_RATE);
            FlipAbstract(MCD_FLIP_PSM_COMP_LEND_DAI_A).file("beg", 103 * WAD / 100);
            FlipAbstract(MCD_FLIP_PSM_COMP_LEND_DAI_A).file("ttl", 6 hours);
            FlipAbstract(MCD_FLIP_PSM_COMP_LEND_DAI_A).file("tau", 6 hours);
            SpotAbstract(MCD_SPOT).file(ILK_PSM_COMP_LEND_DAI_A, "mat", 100 * RAY / 100);
